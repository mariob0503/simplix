var Simulation = (function () {
  ...
  function init() {
    ...
    // Set up GUI (with sensor metrics toggle)
    setupGUI();

    // Bar overlay that fades in (starts at opacity 0; after 3 seconds, transitions to 1 over 4 seconds)
  ...
  function setupGUI() {
    // Define simulation parameters (including sensor metrics toggle)
    params = {
      expansionSpeed: 50,
      particleSize: 1.5,
      bloomStrength: 0.2,
      bloomRadius: 0.05,
      bloomThreshold: 0.095, // *** NEW BLOOM THRESHOLD ***
      showSensorMetrics: false // Toggle for sensor metrics visibility
    };
  ...
  return {
    start: function () {
      // Optionally remove or hide any Lift & Earn UI before starting the simulation
      init();
      animateSimulation();
    },
    stop: function () {
